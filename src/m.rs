...
macro_rules! plist_internal {
    ...
    (@array [$($elems:expr,)*] ? $maybe:expr , $($rest:tt)*) => {
        if let Some(__v) = $crate::m::plist_maybe($maybe) {
    ...
    (@object $object:ident ($($key:tt)+) (:? $value:expr , $($rest:tt)*) $copy:tt) => {
        if let Some(__v) = $crate::m::plist_maybe($value) {
    ...
    (@object $object:ident () ( :< $value:expr , $($rest:tt)*) $copy:tt) => {
        {
            ...
            let __dict = $crate::m::IntoPlistDict::into_plist_dict(__v);
    ...
    (@object $object:ident () ( :< ? $value:expr , $($rest:tt)*) $copy:tt) => {
        if let Some(__dict) = $crate::m::maybe_into_dict($value) {
            for (__k, __val) in __dict {
                let _ = $object.insert(__k, __val);
            }
        }
